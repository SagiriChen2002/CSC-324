      ...
      tabItem(
        ...
        fluidRow(
          column(
            ...
            selectInput("educationAttribute", "Choose Education Level:", choices = c(
              "Percent of adults with less than a high school diploma" = "Percent of adults with less than a high school diploma",
              "Percent of adults with a high school diploma only" = "High school diploma only",
              "Percent of adults with some college or associate's degree" = "Some college or associate's degree",
              "Percent of adults with a bachelor's degree or higher" = "Bachelor's degree or higher"
            )),
            selectInput("covidAttribute", "Choose COVID-19 Attribute:", choices = c(
              "Cases" = "Cases",
              "Deaths" = "Deaths",
              "Vaccine Distributed" = "doses_distributed",
              "Vaccine Administered" = "doses_administered"
...
server <- function(input, output, session) {
  ...
  observeEvent(input$plotGraph, {
    # Fetch and prepare the data based on user selections
    edu_data <- educationData()
    covid_data <- covidData()

    # Assuming the state identifiers match between edu_data and covid_data
    # Extract the relevant columns based on user input for plotting
    plot_data <- edu_data %>%
      select(STUSPS, Education_Level = all_of(input$educationAttribute)) %>%
      left_join(covid_data %>% select(STUSPS, COVID_Attribute = all_of(input$covidAttribute)), by = "STUSPS")
    

    # Generate the plot
    output$linePlot <- renderPlot({
      ggplot(plot_data, aes(x = education_level, y = covid_attribute_value)) +
        geom_line() + # Use geom_point() for scatter plot, geom_bar() for bar plot, etc., depending on your data
        labs(x = input$educationAttributeGraph, y = input$covidAttributeGraph) +
        theme_minimal() +
        ggtitle("Education Level vs. COVID-19 Attribute")
    })
  })
